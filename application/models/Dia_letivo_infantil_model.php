...
class Dia_letivo_infantil_model extends CI_Model
{
			...
			foreach($dados['dias'] as $meses){
				...
				if (!empty($meses[0])){
					...
					foreach($dias as $dia){
						...
						$sql = "INSERT INTO dia_letivo_infantil (dif_dia_letivo)
								VALUES ('". $dados['ano'] . "-" . $dia ."')";

						if(!$this->db->simple_query($sql)){
							$retorno = false;

							$sql = "DELETE FROM dia_letivo_infantil WHERE YEAR(dif_dia_letivo) = ". $dados['ano'];
	...
	function buscarAnoLetivo($dados)
	{
		...
		$sql = "SELECT *  FROM dia_letivo_infantil WHERE YEAR(dif_dia_letivo) =
	...
	function findByAno($ano)
	{
		...
		$sql = "SELECT * FROM dia_letivo_infantil WHERE YEAR(dif_dia_letivo) = $ano";
	...
	function buscarDiaLetivo($dia)
	{
		...
		$sql = "SELECT * FROM dia_letivo_infantil WHERE dif_dia_letivo = '$data'";
